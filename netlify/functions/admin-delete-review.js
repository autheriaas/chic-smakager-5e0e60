...
exports.handler = async function (event) {
  ...
  try {
    const res = await fetch(SHEET_ENDPOINT, {
      method: "POST",
      body: JSON.stringify({ action: "deleteReview", id: body.id, secret: deleteSecret }),
      signal: controller.signal,
    });
    clearTimeout(timeout);
    const data = await res.json().catch(() => ({}));
    if (data.status !== "ok") {
      return { statusCode: 502, headers, body: JSON.stringify({ error: data.message || "Sheet backend refused the delete." }) };
    }
    ...
    return { statusCode: 502, headers, body: JSON.stringify({ error: timedOut ? "The review server took too long to respond." : "Could not reach the review server." }) };
  }
};
